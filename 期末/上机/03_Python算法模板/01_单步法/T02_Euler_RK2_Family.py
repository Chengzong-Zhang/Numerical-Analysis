...
f = lambda t, y: -math.exp(10*t)*(y-math.sin(t))+math.cos(t) # 【必须替换】ODE 右端 f(t,y)；Python 的 ** 表示乘方。
t0, T, y0, h = 0.0, 1.0, 0.0, 0.1  # 【必须替换】区间、初值与固定步长。
method = "trapezoid"  # 【必须替换】可选 forward_euler/backward_euler/trapezoid/improved_euler/midpoint/heun2。
...
for n in range(N):  # 逐步推进到下一节点。
    ...
    if method == "trapezoid":  # 隐式梯形：二阶、A 稳定，来自积分形式的梯形求积。
